...
typedef std::string(Snoopy::* snooper)(neighbors*);// методы поиска единицы
...
typedef  struct {// Структура, отражающая всю информацию о сигнале
	...
	snooper slot;// метод обработчика
	int sig_num;
}to_snp_info_t;
...
typedef std::pair<snooper, to_own_info_t*> to_own_pair_t;// полная информация о связи Snoopy->Owner
